...
def data_description_to_csv(data, target, path, print_summary=False):
    ...
    bool_cols = [
        col
        for col in data
        if data[col].dropna().value_counts().index.isin([0, 1]).all()
    ]
    data_description = data.describe(include="all")

    pvalues = p_values(data, target)
    pvalues[target] = 0.0  # p-value for the target is by definition 0

    # initialize frame header
    header = [
        "Column",
        "All(n={0})".format(data.shape[0]),
        "Missing",
        "Missing(%)",
        "P",
    ]
    ...
    for column in data.columns:
        if column in bool_cols:
            ...
            stats.loc[len(stats)] = [
                ...
                "{0} ({1}%)".format(
                    count,
                    round((count / data[column].dropna().count()) * 100, 2),
                ),
                data[column].isnull().sum(),
                round((data[column].isnull().sum() / data.shape[0]) * 100, 1),
                round(pvalues[column], 3),
            ]
        else:
            stats.loc[len(stats)] = [
                column,
                "{0}+/-{1}".format(
                    round(data_description[column]["mean"], 2),
                    round(data_description[column]["std"], 2),
                ),
                data[column].isnull().sum(),
                round((data[column].isnull().sum() / data.shape[0]) * 100, 1),
                round(pvalues[column], 3),
